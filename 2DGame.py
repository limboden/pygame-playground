...
def shooting_range():
    ...
    while True:
        ...
        for i, projectile in enumerate(projectiles):
            ...
            if projectile[0] < 0 or projectile[0] > WIDTH or projectile[1] < 0 or projectile[1] > HEIGHT:
                del projectiles[i]
            elif math.hypot(projectile[0] - circle_x, projectile[1] - circle_y) <= CIRCLE_RADIUS:
                del projectiles[i]
                circle_health -= random.randint(MIN_WEAPON_DMG, MAX_WEAPON_DMG)
                if circle_health <= 0:
                    circle_health = random.randint(100, 500)
                    circle_x = random.randint(0, WIDTH)
                    circle_y = random.randint(0, HEIGHT)
        ...
        font = pygame.font.Font(None, 36)
        health_text = font.render(f'Health: {circle_health}', 1, (255, 255, 255))
        SCREEN.blit(health_text, (circle_x - 50, circle_y + CIRCLE_RADIUS + 10))
    ...
    while True:
        ...
        for i, projectile in enumerate(projectiles):
            ...
            if projectile[0] < 0 or projectile[0] > WIDTH or projectile[1] < 0 or projectile[1] > HEIGHT:
                ...
                projectile_rect = pygame.Rect(projectile[0], projectile[1], PROJECTILE_SIZE, PROJECTILE_SIZE)
                if projectile_rect.colliderect(pygame.Rect(circle_x, circle_y, CIRCLE_RADIUS * 2, CIRCLE_RADIUS * 2)):
                    damage = random.randint(MIN_WEAPON_DAMAGE, MAX_WEAPON_DAMAGE)
                    circle_health -= damage
                    del projectiles[i]
